...
- [x] **Phase 3: install-hooks UX** ✓ 2026-04-28 — `holt install-hooks` subcommand. Read-merge-write of `~/.claude/settings.json` with `fs2` exclusive lock, JSONC round-trip via `jsonc-parser` CST (preserves comments + key order), `--dry-run` and `--print` escape hatches, `.holt.bak` backup, fsync-before-rename. Verified passed: 5/5 ROADMAP must-haves green, 4/4 reqs (HOOK-07..10), 17/17 decisions, **all 6 hard constraints C1..C6 enforced** (C3 + C4 newly test-enforced this phase via `tests/cli_dep_boundary.rs` and the install-hooks lock module), 80/80 workspace tests post-review-fix.
- [x] **Phase 4.1: Gap-closure (bootstrap automation)** ✓ 2026-04-29 — `tools/bootstrap-github.sh` (one-shot idempotent: gh repo create + first push + setup-labels.sh; preconditions-checked + `--dry-run` mode + `REPO=` override for forks). RC1-CHECKLIST.md Pre-tag section collapsed from 6 steps to 4 (gh-repo-create + label-apply + push merged into one bootstrap call). Tag-push deliberately retained as a separate human decision. Closes the v0.1-MILESTONE-AUDIT.md `gaps_found` status (gap_kind: human_needed_handoff).
- [x] **Phase 4: Distribution + launch** ✓ 2026-04-29 — `dist` v0.31.0 scaffold (Linux x64, macOS x64+arm64, Windows x64 best-effort), `cargo-binstall` metadata via dist auto-gen, MSRV 1.87 / Edition 2024 pin (CI matrix entry), README leading with `vhs`-rendered demo gif (`assets/demo.gif`, 377 KB), `tools/setup-labels.sh` for the 9 CONTRIBUTING.md issue labels, release-workflow `--version` parity check (D-14), `RC1-CHECKLIST.md` maintainer handoff (153 LOC). **Homebrew tap deferred to v0.1.x** (AMENDMENT 2026-04-29) — DIST-02 marked deferred in REQUIREMENTS.md. Verifier PARTIAL — 4/5 ROADMAP success criteria fully verified in the codebase; criterion #2 brew clause DEFERRED via AMENDMENT (override accepted); criterion #4 binstall live-resolve sub-clause structurally correct but pending the documented maintainer rc.1 tag-push. 80/80 workspace tests, all 6 hard constraints C1..C6 enforced. Reviewer PASS-WITH-WARNINGS — 3/4 warnings fixed (WR-01 xattr install path, WR-03 D-14 find robustness, WR-04 vhs hermeticity); WR-02 (binstall README copy) deferred — steady-state-correct post-rc.1.
...
| 3. install-hooks UX | 2/2 | Complete ✓ | 2026-04-28 |
| 4. Distribution + launch | 2/2 | Complete ✓ | 2026-04-29 |
| 4.1. Gap-closure (bootstrap automation) | 1/1 | Complete ✓ | 2026-04-29 |
