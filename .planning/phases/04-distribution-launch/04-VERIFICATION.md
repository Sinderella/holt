...
|---|---------|--------|----------|
| D-01 | Run `dist init` (not hand-write) | **IMPLEMENTED** | `chore(dist): scaffold via dist init v0.31.0`. release.yml header L1–5 self-identifies as autogenerated. |
| D-02 | Ship only holt-cli; library crates excluded | **IMPLEMENTED** | All 5 library crates have `[package.metadata.dist] dist = false`. holt-cli has `dist = true` opt-in. All 6 crates have `publish = false`. |
| D-03 | 4-target matrix; Windows allow-fail | **IMPLEMENTED** | dist-workspace.toml L34 (4 targets). release.yml: per-instance `continue-on-error` expression keyed on Windows triple. |
| D-04 | Homebrew tap = Sinderella/homebrew-holt | **SUPERSEDED-BY-AMENDMENT** | Original tap-pin commit preserved as bisect anchor. Amendment commits (`chore(dist): drop Homebrew tap`, `docs(planning): mark Homebrew tap deferred`) document the v0.1.x defer. Original D-04 instruction wording in CONTEXT.md is overlaid by the AMENDMENT BANNER at the top. |
| D-05 | binstall via dist auto-gen; manual fallback documented | **IMPLEMENTED** | No manual `[package.metadata.binstall]` in holt-cli/Cargo.toml (per design). Plan 04-01's verification step intentionally informational (post-rc.1 verification handled by RC1-CHECKLIST.md). |
| D-06 | Animated GIF via vhs, committed `tools/demo.tape` + `assets/demo.gif` | **IMPLEMENTED** | tools/demo.tape (53 lines, 4-step) + assets/demo.gif (377 KB GIF89a 900×400). |
| D-07 | Platform tier statement above-the-fold + scope.md link | **IMPLEMENTED** | README.md tier paragraph satisfies both clauses. |
| D-08 | MSRV CI matrix entry pinned to 1.87.0, build-only | **IMPLEMENTED** | ci.yml — 3 jobs (linux/macos/windows). Build-only; tests already covered by Phase 1 jobs. |
| D-09 | Idempotent `tools/setup-labels.sh` for 9 labels | **IMPLEMENTED** | tools/setup-labels.sh (50 lines, executable, --force). |
| D-10 | rc.1 readiness via `v0.1.0-rc.1` pre-release tag | **CHECKLIST-IMPLEMENTED** | RC1-CHECKLIST.md authored 2026-04-29 with AMENDMENT-aware steps. Tag-push is the maintainer's manual action. |
| D-11 | Tag-push pattern `v*` | **IMPLEMENTED** | release.yml: `tags: ['v*']`. |
| D-12 | SHA-256 checksums via dist 0.31 default | **IMPLEMENTED** | dist plan output includes `.sha256` sidecars per artifact + `sha256.sum` aggregate + `source.tar.gz.sha256`. |
| D-13 | --version smoke test asserting CARGO_PKG_VERSION | **IMPLEMENTED** | crates/holt-cli/tests/version_smoke.rs using `concat!("holt ", env!("CARGO_PKG_VERSION"))`. Test passes in workspace test run. |
| D-14 | Release-workflow `--version` parity check | **IMPLEMENTED** | release.yml: locates binary, runs `--version`, greps `${GITHUB_REF_NAME#v}`. |
| D-15 | README install-hooks budget reaffirmation | **IMPLEMENTED** | README.md "First-run" section documents `holt install-hooks --dry-run` then `holt install-hooks`, plus `--print` mention; emphasizes atomic mutation, lock, .holt.bak backup, JSONC preservation. |
| D-16 | Phase 4 split into 2 plans (infra → launch UX) | **IMPLEMENTED** | 04-01-PLAN.md (Wave 1, infra) + 04-02-PLAN.md (Wave 2, launch UX). Both have PLAN-CHECK.md PASS verdicts. |
